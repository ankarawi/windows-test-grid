# native_single_case_recovery_node.ps1
# Single-case recovery runner for Batch04 pending cases.
# 1 VM = 1 case = 1 MT5 session = 1 pass.
# Fail-safe encrypted evidence emission on BOTH SUCCESS and CONTROLLED FAILURE.
# Checkpoint telemetry every 5s to local temp file.
# 180-minute (10800s) watchdog. Resource guards (metatester > 16, free RAM < 2000MB, procs > 200).
...
function Emit-EncryptedEvidence {
    try {
        ...
        # Copy telemetry checkpoint log if present
        ...
        # If failure or non-zero, ensure failure summary is written
        if ($script:code -ne 0 -or -not (Test-Path (Join-Path $out 'identity_evidence.json') -PathType Leaf)) {
            $failSummary = [PSCustomObject]@{
                ...
                recovery_id              = $env:GRID_RECOVERY_ID
        ...
        # Never crash inside evidence emission
    }
}

try {
    Write-Host "[GRID] NATIVE SINGLE CASE RECOVERY NODE INITIALIZED"
    $recoveryId = if (-not [string]::IsNullOrWhiteSpace($env:GRID_RECOVERY_ID)) { $env:GRID_RECOVERY_ID.Trim() } else { 'recovery-xx' }
    Write-Host "[GRID] RECOVERY_ID=$recoveryId"
    ...
    # 1. Download and verify encrypted package
    try {
    ...
    $caseDir = Join-Path $payload $recoveryId
    if (-not (Test-Path $caseDir -PathType Container)) {
        Set-Stage 'CASE_DIR_MISSING'; throw "ERR_CASE_DIR_MISSING: $recoveryId"
    }

    $runSpecFile = Join-Path $caseDir 'run_spec.json'
    $setFile     = Join-Path $caseDir "$recoveryId.set"
    $metaFile    = Join-Path $caseDir 'meta.json'

    foreach ($f in @($runSpecFile, $setFile, $metaFile)) {
        if (-not (Test-Path $f -PathType Leaf)) {
            Set-Stage 'CASE_FILE_MISSING'; throw "ERR_CASE_FILE_MISSING: $f"
        }
    }
    ...
    $activeSetPath = Join-Path $profilesTesterDir "$recoveryId.set"
    ...
    $reportXmlName = "opt_report_$recoveryId.xml"
    ...
    if (Test-Path $reportXmlPath) { Set-Stage 'REPORT_CLEAN_FAILED'; throw "ERR_REPORT_PRE_EXISTS" }
...
[Tester]
Expert=worker.ex5
ExpertParameters=$recoveryId.set
Symbol=$symbol
Period=$timeframe
Deposit=$deposit
Currency=$currency
...
"@
    ...
    if ($iniContent -notmatch [regex]::Escape("ExpertParameters=$recoveryId.set")) {
    ...
    # Initialize telemetry checkpoint log
    ...
    while (-not $termProc.HasExited) {
        ...
        if ($script:elapsedSec -ge ($lastSampleSec + $sampleIntervalSec)) {
            ...
            # Append to persistent checkpoint file
    ...
    Get-Process terminal64,metatester64 -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    # MANDATORY: Emit encrypted evidence on BOTH success and failure
